...
- Current registry state is in-memory and single-process only. Do not claim restart recovery or multi-replica consistency until durable shared storage is implemented.
...
Test concurrent assignments, full capacity, stale/replayed heartbeats, Agent loss, group affinity and reservation idempotency. Coordinator restart recovery and multi-replica races require durable shared storage and remain unimplemented.
